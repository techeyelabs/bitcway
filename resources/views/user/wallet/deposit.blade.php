                ...
                <div class="card">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" aria-describedby="" name="amount" id="coinAmount"
                                   value="{{old('amount')}}" placeholder="Enter amount in bitcoin here..." required
                                   v-model="amount" v-on:keyup="hcgenerate">
                            @error('amount')
                            <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="form-group text-center">
                            <h4 class="txtWhitecolor">Equivalent: <span id="amountRate"> @{{amount*rate}}</span> (USD)
                            </h4>
                        </div>
                        <button class="btn btn-outline-warning float-end"
                                onclick="gatewaypost()">{{__('button3')}}</button>
