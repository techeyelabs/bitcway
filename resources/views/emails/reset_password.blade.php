...
@section('content')
    ...
    <p><a href="{{route('reset-action', ['token' => $data->verification_token, app()->getLocale()])}}">ACTIVATE</a></p>
    <p>Or copy the url provided bellow and paste to your browser address bar.</p>
    <p>{{route('reset-action', ['token' => $data->verification_token, app()->getLocale()])}}</p>
@endsection
